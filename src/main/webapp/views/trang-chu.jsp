...
<link rel="stylesheet" href="<c:url value='/templates/css/style1.css'/>">
<link rel="stylesheet"
...
<body>
	...
	<div class="main-content">
		<div class="left-content">
			<div class="post-box">
				<h3 class="box-title">
					<i class="fab fa-hotjar"></i> CHO THUÊ MỚI NHẤT
				</h3>
				<div class="box-content">
					<c:forEach var="post" items="${ptPosts }">
						<div class="post-card">
			...
			<div class="post-box">
				<h3 class="box-title">
					<i class="fas fa-clinic-medical"></i> Ở GHÉP MỚI NHẤT
				</h3>
				<div class="box-content">
				<c:forEach var="plusPost" items="${plusPosts }">
					<div class="post-card">
							<c:forEach var="plusPhoto" items="${photos }">
								<c:if test="${plusPhoto.postId == plusPost.id}">
									<img
										src="<c:url value='/templates/images/${plusPost.id }/${plusPhoto.name }'/>"
										alt="hình ảnh" />
								</c:if>
							</c:forEach>
							<div class="post-info">
								<h3 class="post-title">
									<a href="/chi-tiet?id=<c:out value="${plusPost.id }"/>"><c:out
											value="${plusPost.title }"></c:out></a>
								</h3>
								<hr>
								<p>
									Diện tích: <strong><c:out value="${plusPost.acreage }"></c:out>&nbsp;
										mét vuông</strong>
								</p>
								<p>
									Địa chỉ: <strong><c:out
											value="${plusPost.apartmentNumber }"></c:out>&nbsp; <c:forEach
											var="plusStreet" items="${streets }">
											<c:if test="${plusPost.streetId == plusStreet.id }">
												<c:out value="${plusStreet.name }"></c:out>
											</c:if>
										</c:forEach> </strong>
								</p>
								<p>
									Giá phòng: <strong><fmt:formatNumber type='number'
											maxFractionDigits='3' value='${plusPost.price }' /> đồng/tháng</strong>
								</p>
								<p>
									Ngày đăng: <strong><c:out value="${plusPost.createdDate }"></c:out></strong>
								</p>
								<a href="/chi-tiet?id=<c:out value="${plusPost.id }"/>"><input
									type="button" value="Xem chi tiết"></a>
							</div>
						</div>
				</c:forEach>
